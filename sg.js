...
function Suggest( inputOptions ) {
  ...
  function addEvent( eventType, handler ) {
    Evt.add( self, eventType, handler );
  }
  ...
  function applyOpt( optName, value, prev ) {
    if( /(?:^on[A-Z])/.test( optName.substr( 0, 3 ) ) && isFunction( value ) ) {
      ...
      if( ( eventType = SuggestEvents[ eventType ] ) ) {
        if( prev ) {
...
define(function () {
	...
	Promise.prototype = {
		 ...
		 *    with its reason.
		 */
    // disable because reserved word in strict mode
