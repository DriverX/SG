/*
* YASSMOD - Simple CSS Selector Engine based on YASS 0.3.9
...
var SGUtils = {
	...
	isObj: function ( obj ) {
		...
		if( obj[ str__constructor ] &&
			!hasOwnProperty.call( obj, str__constructor ) &&
			!hasOwnProperty.call( obj[ str__constructor ][ str__proto ], "isPrototypeOf" ) )
		{
			return false;
		}
	...
	format: function( str, replaceObj ) {
		return str && replaceObj ? String( str ).replace( rFormatReplace, function( fullExpr, expr ) {
				return hasOwnProperty.call( replaceObj, expr ) ? replaceObj[ expr ] : "";
			}) : "";
	},
			...
			if( !event ) {
				...
				if( listeners ) {
					for( eventType in listeners ) {
						if( hasOwnProperty.call( listeners, eventType ) ) {
...
function Suggest( inputOptions ) {
	...
	var cache = {
			 ...
			 */
			has: function( key ) {
				return this.c[ key ] != null && hasOwnProperty.call( this.c, key );
			},
	...
	var request = {
			...
			isAct: function( value ) {
				return !arguments.length
					? !!this.act.length
					: !!this.actVals[ value ] && hasOwnProperty.call( this.actVals, value );
			},
			...
			send: function( value, onSuccess, onError ) {
				...
				if( typeof url === "object" ) {
					...
					url = SGUtils.format( url, urlEncode( replacement ) );
				}
...
Suggest.opts = {
	...
	valMax: 255,
	valFilter: /(?:\S)/,
	url: "http://suggests.go.mail.ru/sg_mobi?q={query}",
